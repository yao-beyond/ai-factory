...
<p align="center">
  <img src="docs/screenshots/discovery-flow.png" width="300" alt="Discovery 流程：入口 → 兩個問題 → 成品卡片 → 取名 → 預填需求">
</p>

沒頭緒也能開始，五步就好：

1. **首頁入口** — 點「🤔 還不知道要做什麼？」
2. **兩個小問題** — 給誰用？想做什麼？（用點的，不用打字）
3. **挑一張成品卡片** — 看到喜歡的就說「就是這個！」
4. **取個名字**（選填）— 它就成了你的東西
5. **自動填好需求** — 連能力邊界都幫你寫好，由你**審查後再開工**

> 卡片庫就是 v1 的能力邊界：選得到的都做得出來；做不到的（後端、金流、自動收件）不會出現在選項裡，也不會被偷偷塞進需求。
...
- **成就創意**：讓不懂程式的人，用一句白話就能把想法變成可用的成果。
...
4.  **驗收**：
    *   現有專案：點選「查看成果草案」連結，去 PR 頁面看 AI 寫的程式。
    *   全新專案：點選「⬇️ **下載你的專案**」，並閱讀 AI 附帶的變更摘要。
